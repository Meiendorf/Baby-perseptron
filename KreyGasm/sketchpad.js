window.addEventListener('load', function () {
    ...
    // Clear canvas
    document.getElementById('Cl_But').addEventListener('click', function (event) {
        event.preventDefault();
        clearer();
    }, false)

    // Number recognizer
    document.getElementById('Sub_But').addEventListener('click', function (event) {
        event.preventDefault();
        ...
        //Вызов метода посредством Ajax
        KreyGasm.Neuron.GetAnswerFromBase64(canvas.toDataURL(), Sub_Complete, Sub_Error);
        function Sub_Complete(result) {
            $get("title").textContent = result;
        }
        function Sub_Error(error) {
            alert("Серверная ошибка : " + error._message);
        }
    }, false)
